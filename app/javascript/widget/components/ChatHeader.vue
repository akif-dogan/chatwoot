...
<template>
  <header class="flex justify-between w-full px-5 py-4 gap-2" style="background: linear-gradient(135deg, #6366F1, #A855F7);">
    <div class="flex items-center">
      ...
      <img
        v-if="avatarUrl"
        class="w-8 h-8 ltr:mr-3 rtl:ml-3 rounded-full ring-2 ring-white/30"
        :src="avatarUrl"
        alt="avatar"
      />
      <div class="flex flex-col gap-1">
        ...
        >
          ...
          <div
            :class="`h-2 w-2 rounded-full
              ${isOnline ? 'bg-emerald-400 marsai-status-glow' : 'hidden'}`"
          />
        </div>
        <AvailabilityContainer
          ...
          :show-header="false"
          :show-avatars="false"
          text-classes="text-xs leading-3 text-white/70"
        />
      </div>
    </div>
